...
func TestExtractionIntegration(t *testing.T) {
	...
	t.Run("Basic Endpoint functionality test", getTextFromSelectionUuidBase64NotIncludedAndOwnerUuidAsQueryParam)
}

func getTextFromSelectionUuidBase64NotIncluded(t *testing.T) {
	if *testutil.SkipExtractionTest {
...
func getTextFromSelectionUuidBase64NotIncludedAndOwnerUuidAsQueryParam(t *testing.T) {
	if *testutil.SkipExtractionTest {
		t.Skip("Skipping test due to flags")
	}
	...
	router.ServeHTTP(w, httptest.NewRequest(
		"POST",
		fmt.Sprintf("/api/v1/extract/basic?OwnerUid=%s", uuid.MustParse("ea167a48-c1b3-46c4-911b-090e807132fc")),
		strings.NewReader(string(bytes)),
	))
